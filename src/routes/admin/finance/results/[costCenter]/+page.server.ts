...
import { getTotalsByCostCenter, sumAllResults, keepResultAccounts, accountResult, sumAllAccounts } from '$lib/server/finance/results.js';
...
export async function load({ params }: { params: { costCenter: string } }) {
	...
	const accountBreakdown = Array.from(costCenterResults.values()).map((total) => {
